...
describe "Freelancer view experiences" do
  context "on its profile page" do
    ...
    it "and enters on the experience page" do
      ...
      within "main" do
        expect(page).to have_link "Voltar para #{I18n.t(:profile, scope: "activerecord.models")}", href: profile_path(f_profile)
        expect(page).to have_content "Fast Entregas"
        expect(page).to have_content "Período: #{I18n.l(p_experience.started_at)} - #{I18n.l(p_experience.ended_at)}"
        expect(page).to have_content "Desenvolvedor Full-Stack Junior"
        expect(page).to have_link "Editar #{I18n.t(:experience, scope: "activerecord.models")}", href: edit_profile_experience_path(f_profile, p_experience)
        expect(page).to have_link "Deletar #{I18n.t(:experience, scope: "activerecord.models")}", href: profile_experience_path(f_profile, p_experience)
      end
    end
  end
end