...
describe 'Freelancer register proposal' do
  ...
  context 'unsuccessfully' do
    it 'without filling fields' do
      ...
      within 'form' do
        ...
        fill_in 'Horas semanais disponíveis', with: ''
        fill_in 'Estimativa de entrega', with: ''
        click_on 'Criar Proposta'
      end

      expect(current_path).to eq project_proposals_path(1)
      expect(page).to have_content 'Erro ao criar Proposta!'
      ...
      expect(page).to have_content 'Valor/Hora é muito curto (mínimo: 1 caracter)'
      expect(page).to have_content 'Horas semanais disponíveis não pode ficar em branco'
      expect(page).to have_content 'Estimativa de entrega não pode ficar em branco'
      expect(page).to_not have_link 'Editar Proposta'
      expect(page).to_not have_link 'Deletar Proposta'
      expect(page).to_not have_link 'Aceitar Proposta'
      expect(page).to_not have_link 'Recusar Proposta'
    ...
    it 'filling fields incorrectly' do
      freelancer = Freelancer.create!({ email: 'freelancer@test.com', password: '123456' })
      qa = OccupationArea.create!({ name: 'Quality Assurance' })
      Profile.create!({ full_name: 'Giovanni César Lima', social_name: 'Giovanni César',
                        birth_date: 32.years.ago, formation: 'Ciência da Computação',
      ...
      expect(page).to have_content 'Descrição não pode ficar em branco'
      expect(page).to have_content 'Descrição é muito curto (mínimo: 20 caracteres)'
      expect(page).to have_content 'Valor/Hora deve ser menor ou igual a 300.0'
      expect(page).to have_content 'Horas semanais disponíveis não pode ficar em branco'
      expect(page).to have_content 'Estimativa de entrega não pode ficar em branco'
      expect(page).to_not have_link 'Editar Proposta'
      expect(page).to_not have_link 'Deletar Proposta'
    ...
    it 'because already exists' do
      ...
      proposal = Proposal.create!({ description: 'Sonho com um projeto desses!',
                                    hour_rate: 250, weekly_hours: 40, delivery_estimate: 6.months.from_now,
                                    project: project, freelancer: freelancer })
      ...
      expect(current_path).to eq project_proposal_path(project, proposal)
      expect(page).to have_content "Uma proposta sua já existe para 'Sistema Web'!"
      expect(page).to have_content 'Sistema Web'
      expect(page).to have_content "Proposta de #{freelancer.email}"
      expect(page).to have_content 'Descrição: Sonho com um projeto desses!'
      expect(page).to have_link 'Editar Proposta'
      expect(page).to have_link 'Deletar Proposta'
      expect(page).to_not have_link 'Aceitar Proposta'
