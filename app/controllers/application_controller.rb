class ApplicationController < ActionController::Base
  ...
  def require_log_in!
    if !user_signed_in? && !freelancer_signed_in?
      ...
      allowed_routes = [root_path, new_freelancer_session_path, new_user_session_path,
                        cancel_freelancer_registration_path, new_freelancer_registration_path, edit_freelancer_registration_path, freelancer_registration_path, new_freelancer_password_path, edit_freelancer_password_path, freelancer_password_path, cancel_user_registration_path, new_user_registration_path, edit_user_registration_path, user_registration_path, new_user_password_path, edit_user_password_path, user_password_path]

      unless allowed_routes.include? current_page
  ...
  def freelancer_with_profile?
    if freelancer_signed_in? && current_freelancer.profile.blank?
      ...
      unless allowed_freelancer_routes.include? current_page
        @profile = Profile.new
